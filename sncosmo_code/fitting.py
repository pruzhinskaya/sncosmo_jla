...
from matplotlib import pyplot as plt #maria
...
def generate_chisq(data, model, signature='iminuit', modelcov=False,apply_ZPERR=False):
    """Define and return a chisq function for use in optimization.
    ...
    if modelcov:
        _, mcov = model.bandfluxcov(data.band, data.time,
                                    zp=data.zp, zpsys=data.zpsys,apply_ZPERR=apply_ZPERR)
        cov += mcov
#        print (mcov)
    ...
    **redshift guess:** If redshift (``z``) is being fit and ``guess_z=True``,
    ...
    if method == 'minuit':
        ...
        fitchisq = generate_chisq(fitdata, model, signature='iminuit',
                                  modelcov=False,apply_ZPERR=False)
        ndof = len(fitdata) - len(vparam_names)
        ...
        while refit:
            ...
            fitchisq = generate_chisq(fitdata, model, signature='iminuit',
                                      modelcov=modelcov,apply_ZPERR=apply_ZPERR)
