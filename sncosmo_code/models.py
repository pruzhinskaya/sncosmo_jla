...
#        scale = self._model['errscale'](phase, band.wave_eff)[:, 0]
        ...
        lcrv00 = self._model['LCRV00'](phase, band.wave_eff_snfit(EBV=self.EBV_snfit,z=self.z_snfit,Rv=self.Rv_snfit))[:, 0] # masha
        lcrv11 = self._model['LCRV11'](phase, band.wave_eff_snfit(EBV=self.EBV_snfit,z=self.z_snfit,Rv=self.Rv_snfit))[:, 0] # masha
        lcrv01 = self._model['LCRV01'](phase, band.wave_eff_snfit(EBV=self.EBV_snfit,z=self.z_snfit,Rv=self.Rv_snfit))[:, 0] # masha
        scale = self._model['errscale'](phase, band.wave_eff_snfit(EBV=self.EBV_snfit,z=self.z_snfit,Rv=self.Rv_snfit))[:, 0] # masha
...
#################################################Florian#############################
    ...
    def bandfluxcov(self, band, time, zp=None, zpsys=None, apply_ZPERR=False):
