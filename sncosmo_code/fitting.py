...
def generate_chisq(data, model, signature='iminuit', modelcov=False, apply_ZPERR=False): # masha
    ...
    if modelcov:
        _, mcov = model.bandfluxcov(data.band, data.time,
                                    zp=data.zp, zpsys=data.zpsys, apply_ZPERR=apply_ZPERR) # masha
        cov += mcov
    ...
    **redshift guess:** If redshift (``z``) is being fit and ``guess_z=True``,
    ...
    if method == 'minuit':
        ...
        fitchisq = generate_chisq(fitdata, model, signature='iminuit',
                                  modelcov=False, apply_ZPERR=False) # masha
        ndof = len(fitdata) - len(vparam_names)
        ...
        while refit:
            ...
            fitchisq = generate_chisq(fitdata, model, signature='iminuit',
                                      modelcov=modelcov, apply_ZPERR=False) # masha
