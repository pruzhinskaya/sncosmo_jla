...
def fit_salt2(results=False):
    ...
    #list_jla = ['lc-SDSS16281.list', 'lc-04D4ic.list']
    list_jla = os.listdir('jla_data/jla_light_curves/')
    fitfail = []
    for filename in list_jla:
        if filename.startswith('lc-sn'):
            sn_name = filename
            print sn_name
...
#            mb = model2.bandmag('jla_STANDARD::B', 'jla_VEGA2_mb', [res.parameters[1]])

#            dbmag_pecvel = -5*np.log10((1+head['@Z_HELIO'])/(1+head['@Z_CMB']))

#            print dbmag_pecvel, head['@Z_HELIO'], head['@Z_CMB']
